...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		if(word.length()==1 && checkIfVowel(word, 0))	return 1; // for one-letter-word
		...
		for(int i=0; i<word.length(); i++) {
			...
			if(checkIfVowel(word, i)) {
				...
				while(j<word.length() && checkIfVowel(word, j)) {
					j++;
				}
		...
		if(numSylls > 1 && word.charAt(word.length()-1) == 'e' && !checkIfVowel(word,word.length()-2)) {
//			System.out.println("Word: \""+ word+ "\" => Checking ending e;");
			numSylls--;
		}
...
//		System.out.println("\n\nSentences, words, syllables: "+getNumSentences()+", "+getNumWords()+", "+getNumSyllables());
//		System.out.println("MY FLESCH: "+flesch);
	    return flesch;
	}
	
	private boolean checkIfVowel(String word, int index) {
		char c = java.lang.Character.toLowerCase(word.charAt(index));
		if(c == 'a' || c == 'e' || c == 'i' || 
				  c == 'o' || c == 'u' || c == 'y')
			return true;
		else return false;
